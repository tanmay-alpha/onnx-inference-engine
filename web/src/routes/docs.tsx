  ...
  head: () => ({
    meta: [
      { title: "Operator Docs · Crucible" },
      {
        name: "description",
        content:
          "Reference documentation for every ONNX operator implemented in the Crucible engine.",
      },
    ...
    <CrucibleLayout>
      <section className="c-container">
        <div style={{ marginBottom: 32, maxWidth: 720 }}>
          ...
          <p className="c-muted">
            Every operator implemented in the Crucible core, with C++ signatures and FFI examples.
          </p>
        </div>
        ...
        <div className="c-docs">
          <aside className="c-sidebar">
            ...
            {CATEGORIES.map((c) => (
              ...
              >
                {c.label}{" "}
                <span style={{ color: "var(--ink-muted)", marginLeft: 6 }}>({c.ops.length})</span>
              </button>
            ))}
          </aside>
          ...
          <div>
            {CATEGORIES.map((c) => (
              <div
                key={c.key}
                id={`op-cat-${c.key}`}
                style={{ scrollMarginTop: 100, marginBottom: 40 }}
              >
                <h2 className="c-h2" style={{ fontSize: 24, marginBottom: 16 }}>
                  {c.label}
                </h2>
                {c.ops.map((op) => (
                  <div key={op.name} className="c-op-card">
                    ...
                    <div className="c-grid-2">
                      <div>
                        ...
                        <div className="c-muted mono" style={{ marginTop: 4 }}>
                          {op.inputs}
                        </div>
                      </div>
                      <div>
                          ...
                          <tbody>
                            {op.attrs.map((a) => (
                              <tr key={a.name}>
                                ...
                                <td style={{ color: "var(--ink-muted)", fontFamily: "inherit" }}>
                                  {a.note}
                                </td>
                              </tr>
                            ))}
                    ...
                    <div style={{ marginTop: 14 }}>
                      ...
                      <pre className="c-code" style={{ marginTop: 6 }}>
                        {op.usage}
                      </pre>
                    </div>
                  </div>
